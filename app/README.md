...
目标是先跑通一个最小 App：月光陪伴体全屏展示、轻量状态切换、麦克风入口、声音波形占位和可展开的文字对话面板。当前阶段不接真实对话模型、语音识别、长期记忆或精准口型。完整路线见 [docs/plan.md](../docs/plan.md)。
...
- AI 灵魂粒子能量体铺满手机屏幕，使用 Canvas 2D 实时渲染。
- 右上角半透明状态下拉框，可切换 `idle`、`listening`、`thinking`、`speaking`、`happy`、`comforting`。
- 左上角“对话”按钮，点击后弹出文字对话面板。
- 底部麦克风按钮，点击后进入/退出模拟录音状态。
- 录音状态下显示声音波形动画，并自动切换到 `listening`。
- 停止录音后自动切换到 `thinking`，作为后续接入识别和模型回复的占位流程。

## 视觉路线

当前主视觉是 [月光陪伴体](../docs/moonlight-plan.md)：

- 主体：Canvas 粒子能量体。
- 主色调：深蓝、青蓝、冰蓝，少量暖金点缀。
- 风格目标：有机、灵动、流体、带水墨感的能量汇聚。
- 实现方式：Canvas 2D 实时渲染，使用向量场、呼吸调制、lighter 发光混合和拖尾。

视觉控制接口：

- `agitate()`：让粒子瞬间剧烈乱颤，适合未来 AI 说话时调用。
- `calmDown()`：恢复缓慢呼吸。
- `changeMood(color)`：改变整体粒子色调。

## 视频素材

早期人形视频 loop 已归档为实验路线。相关文件仍保留在：
